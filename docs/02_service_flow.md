...
1. 오늘 날짜 간지 계산
2. 사용자 사주 간지 데이터 조회
3. fortune_phrases 테이블에서 문장 랜덤 조회
4. Vector DB에서 사주 해석 문서 검색 (RAG)
5. LLM 프롬프트 생성
6. LLM 호출
7. AI 운세 문장 생성
8. DB 문장 + AI 문장 결합
9. 운세 점수 생성 (종합/재물/연애/건강/직장)
10. 행운 아이템 생성
11. daily_fortunes 테이블 저장
12. fortune_logs 테이블에 프롬프트/응답 로그 저장
...
푸시 알림은 대표 프로필(is_primary = true)을 기준으로 발송된다.

### 푸시 알림 과정

1. 매일 아침 스케줄러 실행
2. is_primary = true 프로필 조회
3. 해당 프로필의 오늘 운세 조회
4. 운세 요약 메시지 생성
5. push_subscriptions 테이블에서 fcm_token 조회
6. 사용자 기기로 푸시 알림 발송
